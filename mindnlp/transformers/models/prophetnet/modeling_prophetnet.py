...
def compute_relative_buckets(num_buckets, max_distance, relative_positions, is_bidirectional=False):
    ...
    if is_bidirectional:
        ...
        inv_relative_positions = ops.obs(inv_relative_positions)
    else:
...
class ProphetNetAttention(nn.Cell):
    ...
    def _shape(self, tensor: Tensor, seq_len: int, bsz: int):
        return tensor.view(bsz, seq_len, self.num_attn_heads, self.head_dim).swapaxes(1, 2).contiguous()
...
class ProphetNetFeedForward(nn.Cell):
    """
...
class ProphetNetNgramSelfAttention(nn.Cell):
    ...
    def _shape(self, tensor, seq_len, batch_size):
        return tensor.view(batch_size, seq_len, self.num_attn_heads, self.head_dim).swapaxes(1, 2).contiguous()
    ...
    def get_main_relative_pos_embeddings(
        ...
        main_relative_pos_embeddings = ops.gather_elements(rel_pos_embeddings, dim=1, index=main_relative_position_buckets)
...
class ProphetNetEncoderLayer(nn.Cell):
    """
...
class ProphetNetDecoderLayer(nn.Cell):
    """
...
class ProphetNetEncoder(ProphetNetPreTrainedModel):
    ...
    def __init__(self, config: ProphetNetConfig, word_embeddings: nn.Embedding = None):
        ...
        self.layers = nn.CellList([ProphetNetEncoderLayer(config) for _ in range(config.num_encoder_layers)])
...
class ProphetNetDecoder(ProphetNetPreTrainedModel):
    ...
    def __init__(self, config: ProphetNetConfig, word_embeddings: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.CellList([ProphetNetDecoderLayer(config) for _ in range(config.num_decoder_layers)])
        self.embeddings_layer_norm = LayerNorm(config.hidden_size)
